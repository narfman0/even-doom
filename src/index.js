import { waitForEvenAppBridge, TextContainerProperty, CreateStartUpPageContainer, OsEventTypeList, ImuReportPace, } from '@evenrealities/even_hub_sdk';
...
async function main() {
    ...
    async function pushText(content) {
        await bridge.textContainerUpgrade({ containerID: CONTAINER_ID_GAME, content });
    }
    await pushText('DOOM');
    ...
    function tick() {
        ...
        if (state.moveForward)
            raycaster.moveForward(dt);
        if (state.moveBack)
            raycaster.moveBack(dt);
        if (state.shoot)
            raycaster.shoot();
